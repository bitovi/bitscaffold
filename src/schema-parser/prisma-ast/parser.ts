...
export class PrismaParser extends CstParser {
  ...
  private fieldrules = this.RULE('fieldrules', () => {
    this.CONSUME(lexer.Identifier, { LABEL: 'fieldName' });
    this.OPTION1(() => {
      this.OR([
        { ALT: () => this.CONSUME(lexer.Array, { LABEL: 'array' }) },
  ...
  private block = this.RULE('block', ({ componentType } = { componentType: "" }) => {
    const isEnum = componentType === 'enum';
    const isModel = componentType === 'model';
    const isValidate = componentType === 'validate';
    ...
    this.MANY(() => {
      this.OR([
        ...
        {
          GATE: () => isValidate,
          ALT: () => this.SUBRULE(this.fieldrules, { LABEL: 'list' }),
        },        
        {
          GATE: () => isEnum,
  ...
  private component = this.RULE('component', () => {
    const type = this.OR1([
      ...
      { ALT: () => this.CONSUME(lexer.Validate, { LABEL: 'type' }) },
    ]);
    this.OR2([
      {
        ALT: () => {
