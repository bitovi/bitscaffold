...
describe("Demo", () => {
    ...
    it("should show some basic functionality", async () => {
        ...
        const result1 = await request(server).post("/api/Team").set('authorization', "test").send({
            name: "Team " + new Date(),
        });
        ...
        const result2 = await request(server).post("/api/Player").set('authorization', "test").send({
            teamId: 1,
        ...
        const result3 = await request(server).get('/api/Player').set('authorization', "test");
        ...
        const result4 = await request(server).get('/api/Team').set('authorization', "test");
